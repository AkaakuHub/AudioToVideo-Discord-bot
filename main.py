...
async def on_message(message):
    ...
    if message.attachments:
        for attachment in message.attachments:
            for extension in (".mp3", ".wav", ".m4a", ".aac", ".flac", ".ogg", ".wma"):
...
# 削除に失敗してキャッシュファイルが90を超えた場合、古いファイルから3つずつ削除
async def delete_old(message):
    try:
        ...
        for extension in delete_ext:
            file_list.extend(
                glob.glob(
                    os.path.join("/home/runner/AudioToVideoBot/", f"*{extension}")
                )
            )
...
async def send_video(message, unq_name):
    ...
    try:
        ...
        await delete_data(message, unq_name)
    except FileNotFoundError:
        print(f"Error: File not found: {file_path}")
        await message.channel.send("エラー4が発生しました。開発者にお問い合わせください。\nAn error 4 has occured. Please contact the developer.")
        
#ストレージとプライバシーの観点から、使用済みデータをすぐに削除する
async def delete_data(message, unq_name):
    try:
        file_list = glob.glob(os.path.join("/home/runner/AudioToVideoBot/", f"*{unq_name}*"))
        if not file_list:
            print("削除対象のファイルが見つかりませんでした。")
            await message.channel.send("エラー5が発生しました。開発者にお問い合わせください。\nAn error 5 has occured. Please contact the developer.")
        ...
        for file_to_remove in file_list:
            try:
                ...
                print(f"ファイルの削除中にエラーが発生しました: {e}")
                await message.channel.send("エラー6が発生しました。開発者にお問い合わせください。\nAn error 6 has occurred. Please contact the developer.")
    except Exception as e:
        print(f"エラーが発生しました: {e}")
        await message.channel.send("エラー7が発生しました。開発者にお問い合わせください。\nAn error 7 has occurred. Please contact the developer.")
